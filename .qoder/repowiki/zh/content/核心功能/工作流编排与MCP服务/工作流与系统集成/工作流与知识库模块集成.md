...
- [buidai.vue](file://pages/buidai.vue#L1-L800)
...
RAG（检索增强生成）检索节点的嵌入是工作流系统的关键特性。在`pages/buidai.vue`中，平台明确提出了"创新 RAG 检索"的概念，并将其与"可视化 Workflow 编排"和"超易用 AI 知识库"并列作为核心卖点。
...
基于`pages/buidai.vue`中的实际应用场景，我们可以分析端到端的数据流，包括文档切片、向量存储查询、相关性排序和结果注入等环节的技术实现。
