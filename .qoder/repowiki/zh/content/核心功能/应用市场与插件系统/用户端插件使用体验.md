...
## 引言
本文件面向最终用户，系统性梳理在 智言万象 平台上“发现—安装—配置—使用”的完整插件体验流程。围绕应用市场页面的界面布局、分类导航、搜索过滤与推荐逻辑，指导用户通过“一键安装”启用插件，并在 AI 对话与工作流中自然调用其能力。同时结合场景化用例，说明插件如何无缝融入日常工作流，例如通过自然语言触发天气查询或快递跟踪等常见任务。文档还覆盖权限提示、更新通知、使用统计与卸载反馈等交互细节，确保体验流畅透明。

## 项目结构
智言万象 前端采用 Nuxt 4 + Vue 3 构建，应用市场页面位于 pages/plugin.vue，插件数据与场景数据分别来自 utils/pluginData.ts 与 utils/scene.ts，首页落地页组件（如 HeroSection、ScenarioSection）提供插件生态与场景化展示，工作流与智能体页面（pages/智言万象.vue、pages/agent.vue）体现插件在 AI 对话与自动化流程中的集成方式。
...
B["pages/智言万象.vue<br/>平台核心能力与工作流"]
...
- [pages/智言万象.vue](file://pages/智言万象.vue#L717-L916)
...
participant B as "平台核心能力<br/>pages/智言万象.vue"
U->>P : 打开应用市场
...
- 结合 pages/智言万象.vue 与 pages/agent.vue 的能力描述，用户可在自然语言中触发插件功能，例如：
...
- 平台能力
  - pages/智言万象.vue 与 pages/agent.vue 展示工作流与智能体能力，体现插件在AI对话与自动化流程中的集成方式。
...
## 结论
智言万象 的插件使用体验以“应用市场—生态展示—工作流集成”为主线，通过直观的搜索与分类导航、自动同步的生态展示与场景化用例，帮助用户快速发现并启用插件。在 AI 对话与工作流中，插件以工具或API的形式被自然调用，从而无缝融入用户的日常工作流。平台在性能与交互上提供了良好的基础，用户可通过上述流程获得顺畅、透明的插件使用体验。
