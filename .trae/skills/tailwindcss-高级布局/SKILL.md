---
name: TailwindCSS-高级布局
description: "用于复杂响应式布局的高级CSS网格、Flexbox和容器查询模式。
涵盖CSS网格的掌握，包括圣杯布局、网格模板区域、自动填充/自动拟合以及嵌套对齐的子网格 Flexbox图案用于空间分配，物品尺寸灵活，布局类似砌体，并配有适当的溢出处理 具备命名容器和容器查询单元的容器查询，实现组件级响应性，独立于视口 粘性和固定位置策略、z索引分层系统、滚动快卡轮播和自定义滚动条样式 高级间距技术包括逻辑性质、出血的负边距、多列文本布局以及用夹具进行流体尺寸调整"
---
...
4. 测试所有断点
为所有响应式布局创建系统测试，确保它们在每个断点都能正常工作。